...
def label_candles(df, forward_window=10, rr_ratio=2.0):
    """
    Label setiap candle: 0 (BUY), 1 (SELL), 2 (HOLD)
    Target: Profit TP=2*ATR, Stop Loss SL=1*ATR dalam 10 candle ke depan
    """
    atr = df['atr'].values
...
def process_and_save():
    ...
    print("Processing features for M15...")
    ...
    # Save processed data
    df.to_sql("processed_m15", conn, if_exists='replace', index=False)
    print(f"Saved {len(df)} records to processed_m15")
